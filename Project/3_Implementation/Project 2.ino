...
void setup() 
{
  ...
  Serial.begin(9600);
  lcd.begin(16,2);  
  lcd.print("Be Carefull,Accident Alert");
  lcd.setCursor(0,1);
  lcd.print("System");
  delay(2000);
  lcd.clear();
...
void show_coordinate()
{
    ...
    Serial.print("Speed of the vehicle(in knots)=");
    Serial.println(Speed);
    delay(2000);
    lcd.clear();
    lcd.print("Speed of the vehicle(Knots):");
    lcd.setCursor(0,1);
    lcd.print(Speed);
}
void coordinate2dec()
{
  ...
  String log_minute="";
    for(i=35;i<=41;i++)
      log_minute+=gpsString[i];
    
    Speed="";
    for(i=45;i<48;i++)          //extract longitude from string
      Speed+=gpsString[i];
      
     float minute= lat_minute.toFloat();
     minute=minute/60;
     float degree=lat_degree.toFloat();
     latitude=degree+minute;
     
     minute= log_minute.toFloat();
     minute=minute/60;
     degree=log_degree.toFloat();
     logitude=degree+minute;
}
void Send()
{ 
   Serial1.println("AT");
   ...
   Serial1.print('"');
   Serial1.print("9944476912");    //mobile no. for SMS alert
   Serial1.println('"');
   delay(500);
   serialPrint();
